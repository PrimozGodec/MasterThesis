...
\section{Primerjava pristopov glede na naravnost barvanja}

Izkaže se, da številčna napaka ni vedno dobra metrika za kvaliteto barvanja, saj bolj kot natančno obarvane slike želimo slike, ki izgledajo naravno. Številčna napaka ni najboljša metrika predvsem v naslednjih primerih:
...
\item Odtenki na originalni sliki so lahko različno močni, medtem ko barvanje lahko izgleda enako naravno pri različno močnih barvah. Na primer travnik je lahko obarvan z bolj nežnimi sepia barvami in ali bolj močno zeleno barvo. Oboje izgleda naravno, medtem ko številčna napaka preferira eno od teh rešitev ali nekaj vmes. 

\end{itemize}

Zaradi omenjenih problemov s številčno napako smo se odločili, da pristope evalviramo tudi s pomočjo ljudi, ki so bolj primerni za ocenjevanje naravnosti barv na sliki. 

Podatke smo zajemali s spletno anketo. Anketirancem smo na zaslonu pokazali eno sliko obarvano z dvema različnima pristopoma, ti pa so morali oceniti, katera slika je bolje oziroma bolj naravno obarvana. Slika \ref{im:evalvation-screen} prikazuje izgled aplikacije za evalviranje. Vsak anketiranec je ocenil $23$ parov slik od katerih sta bili dve testni in nista upoštevani v evalvaciji. 
Ostalih $21$ je izbranih tako, da je vsak od anketirancev vsako od kombinacij pristopov enkrat. Anketiranec je vedno dobil drugo sliko, tako da smo zmanjšali vpliv že videne na oceno. Slike, smo izbirali po principu naključne zasnove poizkusa (ang. {\em randomized experimental design}) \cite{wu2006sampling}. Za tako zasnovo smo se odločili, da izničimo vse vplive, ki bi jih lahko povzročil vrstni red slik. 
...
\end{center}
\caption{Zaslon aplikacije za evalviranje kakovosti barvanja slik s pomočjo ljudi. Anketiranec je v posameznem koraku dobil na zaslon sliko obarvano z dvema pristopoma, ki s klikom na sliko je ocenil, katera je bolje in bolj naravno obarvana. }
...
Za primerjavo pristopov smo zgradili Bradley–Terry model \cite{hunter2004mm}. Ta parametre oceni z metodo največjega vrjetja (ang. {\em  maximum likelihood}). Model je zgrajen tako, da nam je v pomoč pri rangiranju podatkov pridobljenih v medsebojni primerjavi parov. Pri katerem ni nujno, da so primerjave popolne in enako pogoste za pare.

Model je bil zgrajen na podlagi primerjav prikazanih v tabeli \ref{tab:rez-survey} v prilogi. Izračunani so bili parametri model $\gamma_i$ in $\beta_i$, ki so si v razmerju $\gamma_i = e^{\beta_i}$, kjer je $i$ zaporedna številka pristopa. Večja vrednost parametra pomeni pristop, ki se je bolje izkazal glede na evalvacijo. Parametri so prikazani v tabeli \ref{tab:rang-survey}.
...
\begin{center}
    \begin{tabular}{lccc}
        ...
        \hline
        Klas. brez uteži - plitva arh. & 0.098 & -2.32 \\
        Klas. z utežmi - plitva arh. & 0.076 & -2.58 \\         
       	\hline
    \end{tabular}
\end{center}
...
Če rezultate primerjamo s tistimi v poglavju \ref{ch:rez-vecji}, kjer smo pristope primerjali glede na izračunano napako, vidimo določene podobnosti in tudi razlike. Najbolje se je enako kot pri številčni napaki izkazal pristop Iizuka in sod.  Zaostaja naš pristop z regresijo na celih sliki, do spremembe pa je prišli pri pristopu z regresijo na celih slikah z mrežo VGG, ki se je glede na oceno ljudi izkazal za boljšega od regresije po delih. Enako kot pri številčni napaki je pristop, ki ga je implementiral Dalh boljši od klasifikacijskih, ki se tudi v primeru evalvacije s pomočjo ljudi izkažeta za slabša. 
...
\begin{tabular}{p{1.8cm}ccccccc}
	\hline
          & \rot[90]{Iizuka in sod.} & \rot[90]{Dahl}      & \rot[90]{Reg. po delih} & \rot[90]{Reg. celotna slika} &\rot[90]{Reg. celotna VGG} & \rot[90]{Klas. brez uteži - plitva arh.} & \rot[90]{Klas. z utežmi - plitva arh.} \\
\hline
