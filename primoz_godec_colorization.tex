...
V tem poglavju predstavljamo arhitekture nevronskih mrež, ki smo jih načrtovali v okviru magistrskega dela, pogledali si bom pristope z regresijo in klasifikacijo in predstavili postopek učenja. Opisan je poizkus z barvanjem slik večjih od tistih na katerih je bila mreža naučena, predstavljeni pa so tudi učni in testni podatki ter način evalvacije. 
...
Pri enem od klasifikacijskih pristopov smo uporabili cenilno funkcijo Kullback-Leibler divergenca \cite{joyce2011kullback}, ki jo označujemo z oznako $KL-divirgenca$. V našem primeru smo izvedli Kullback-Leibler divergenco na posamezni slikovni točki in jo povprečili preko celote slike kot prikazuje enačba \ref{eq:kld}. Oznake v enačbi so enake oznakam v enačbi \ref{eq:mse}, razen oznaka $c$ v tem primeru predstavlja število razredov, v katere mreža klasificira barve, zato je $c = 400$. 
...
Pri učenju smo beležili vrednosti cenilne funkcije po vsakem končanem prehodu čez vse podatke (ang. {\em epoch}) na množici za učenje in množici za validacijo. S tem smo opazovali, kdaj je določen pristop optimalno naučen. To se v našem primeru zgodi v trenutku, ko vrednost cenilne funkcije na validacijski množici prenehajo padati ali začne naraščati. V tem trenutku je naša mreža optimalno naučena, zato smo te uteži uporabili za testiranje. Grafi padanja cenilnih funkcij za vse pristope so prikazani na sliki \ref{im:histograms-100}. 
...
\end{center}
\caption{Prikaz padanja napake modela pri učenju na manjši množici. Za vsak prehod preko vseh podatkov (ang. {\em epoch}) je prikazana vrednost cenilne funkcije na množici za učenje in validacijski množici. }
...
Izbrali smo dva klasifikacijska pristopa klasifikacija brez uteži - plitva arhitektura in klasifikacija z utežmi - plitva arhitektura, namreč izkazalo se je, da globoka arhitektura poslabša rezultate, zato smo se odločili, da izvedemo le primerjavo dveh plitvih arhitektur. 
